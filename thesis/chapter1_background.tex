...
\textbf{POSIX Threads} provides a standard interface for performing mulithreaded computation. 
...
The efficiency of an application improves when it designed with threads because they have their
own stack and status. The overhead of creating a separate process can be avoided here.
Resources like files are shared among threads. Though threads are good alternatives for
having multiple processes in a single processor machine it is very difficult to scale
it to multi-core processors.
